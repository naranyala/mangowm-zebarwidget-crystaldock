...
static gboolean on_render(GtkGLArea *area, GdkGLContext *context, gpointer user_data) {
    ...
    float vertices[NUM_BANDS * 4]; // 2 vertices per band (bottom and top)
    pthread_mutex_lock(&audio_mutex);
    for (int i = 0; i < NUM_BANDS; i++) {
        float x = -1.0f + (2.0f * i) / (NUM_BANDS - 1);
        
        // Bottom vertex
        vertices[i*4] = x;
        vertices[i*4 + 1] = -1.0f;
        
        // Top vertex (clamp to 1.0f max height)
        float height = eq_bands[i] * 10.0f; // Scale factor for visuals
        if (height > 2.0f) height = 2.0f;
        
        vertices[i*4 + 2] = x;
        vertices[i*4 + 3] = -1.0f + height;
    }
    pthread_mutex_unlock(&audio_mutex);
    ...
    glLineWidth(4.0f); // Make EQ bars thick
    glDrawArrays(GL_LINES, 0, NUM_BANDS * 2);
