// ocws-speaker-qs: OpenGL ES speaker visual widget for OCWS (Quickshell-enabled)
//
// Same GTK4 GtkGLArea / OpenGL ES speaker widget as ocws-speaker-gl, but
// it ALSO publishes the live left/right levels and active stream as JSON to
// $XDG_RUNTIME_DIR/ocws-speaker-qs.json, so the Quickshell QML widget
// (OcwsAudioQS.qml) can render the same animation inside a panel.
//
// Build: see build.zig (ocws-speaker-qs target). Run: ocws-speaker-qs
...
static float g_pubL = 0.0f;   // smoothed levels for JSON publish
static float g_pubR = 0.0f;
...
static GLuint compile(GLenum type, const char *src) {
    ...
    if (!ok) {
        ...
        g_printerr("ocws-speaker-qs: shader compile error: %s\n", log);
    }
    return sh;
}
...
static void build_program(int is_es) {
    ...
    if (!linked) {
        ...
        g_printerr("ocws-speaker-qs: program link error: %s\n", log);
...
static void on_realize(GtkGLArea *area, gpointer data) {
    ...
    if (gtk_gl_area_get_error(area)) {
        g_printerr("ocws-speaker-qs: failed to create GL context: %s\n",
            gtk_gl_area_get_error(area)->message);
        return;
    }
    const char *ver = (const char *)glGetString(GL_VERSION);
    int is_es = ver && strstr(ver, "OpenGL ES") != NULL;
    g_print("ocws-speaker-qs: GL context: %s (using %s shaders)\n",
        ver ? ver : "?", is_es ? "OpenGL ES" : "desktop GL");
    build_program(is_es);
    if (!g_program)
        g_printerr("ocws-speaker-qs: shader program unavailable; showing fallback.\n");
}
...
static gboolean on_render(GtkGLArea *area, GdkGLContext *ctx) {
    ...
    if (strcmp(active, last_active) != 0) {
        ...
        char title[300];
        snprintf(title, sizeof(title), "OCWS Speaker QS%s%s",
            *active ? " — " : "", active);
...
// ---- JSON publish (for the Quickshell widget) ---------------------------
...
static void state_path(char *path, size_t n) {
    ...
    if (rt && *rt)
        snprintf(path, n, "%s/ocws-speaker-qs.json", rt);
    else
        snprintf(path, n, "/tmp/ocws-speaker-qs.json");
}

static gboolean publish_cb(gpointer data) {
    (void)data;
    ...
    char active[256];
    json_safe(audio_stream_active(), active, sizeof(active));

    FILE *f = fopen(tmp, "w");
    if (f) {
        fprintf(f, "{\"l\":%.4f,\"r\":%.4f,\"active\":\"%s\"}\n", g_pubL, g_pubR, active);
        fclose(f);
        rename(tmp, path);
    }
...
static void activate(GtkApplication *app, gpointer data) {
    (void)data;
    g_print("ocws-speaker-qs: window activating...\n");
    GtkWidget *window = gtk_application_window_new(app);
    g_window = window;
    gtk_window_set_title(GTK_WINDOW(window), "OCWS Speaker QS");
    ...
    // Publish levels for the Quickshell widget at ~30 fps.
    g_timeout_add(33, publish_cb, NULL);
}
...
int main(int argc, char **argv) {
    ...
    if (audio_stream_init() != 0)
        g_printerr("ocws-speaker-qs: audio module failed to init; running on idle animation.\n");

    GtkApplication *app = gtk_application_new("org.ocws.speakerqs", G_APPLICATION_DEFAULT_FLAGS);
